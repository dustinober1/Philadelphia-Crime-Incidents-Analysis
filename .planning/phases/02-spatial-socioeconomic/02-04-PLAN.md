...
**6.1 Calculate District Population Estimates**
```python
# Use district-level population from census tract aggregation or external source
# For now, use a proxy: normalize by area or use census data

# Option 1: Use area-based proxy (crimes per sq km)
...
# Note: True per-capita requires population by district (will use area as proxy if population unavailable)
...
district_stats['composite_score'] = (
    ...
    WEIGHTS['area_rate'] * district_stats['area_rate_score']
)
...
districts_scored = districts_gdf.merge(district_stats[['dc_dist', 'severity_score', 'total_crimes', 
                                                        'violent_ratio', 'yoy_change', 'crimes_per_km2']], 
                                        on='dc_dist', how='left')
```
...
ranking_table = district_stats[['dc_dist', 'severity_score', 'total_crimes', 
                                 'violent_ratio', 'yoy_change', 'crimes_per_km2']].copy()
...
ranking_table['crimes_per_km2'] = ranking_table['crimes_per_km2'].round(0)
...
with open(REPORTS_DIR / 'district_severity_ranking.md', 'w') as f:
    ...
    f.write(f"- Crimes per km² (weight: {WEIGHTS['area_rate']})\n\n")
    f.write("## Ranking\n\n")
    f.write(md_table)
...
- [ ] Ranking includes all four factors (count, violent %, YoY, rate)
...
- 02-01 infrastructure: police_districts.geojson, spatial_utils.py, phase2_config_loader.py
