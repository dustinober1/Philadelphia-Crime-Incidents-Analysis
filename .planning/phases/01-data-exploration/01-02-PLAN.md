...
<task type="auto">
  ...
  <action>
    ...
    - Implement `get_summary()`: returns shape, columns, memory usage.
...
<task type="auto">
  ...
  <verify>python -c "import pandas as pd; from src.analysis.profiler import DataProfiler; df = pd.DataFrame({'num': [1, 2, 3, 100], 'cat': ['a', 'a', 'b', 'c']}); p = DataProfiler(df); print(p.check_outliers()); print(p.check_categorical_breakdown())"</verify>
  <done>Outliers (100) detected, categorical counts returned</done>
</task>
...
<success_criteria>
- DataProfiler class implements all required checks (DATA-02, DATA-03, DATA-04)
